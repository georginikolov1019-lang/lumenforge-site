      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-3 max-w-lg text-stone-400">Mock gallery cards — cinematic gradients standing in for finished renders.</p>
          </div>
          <a href="#cta" className="text-sm text-amber-glow transition hover:text-amber-soft">Generate your first reel →</a>
        </div>
            ...
            <figure key={f.title} className="group overflow-hidden rounded-2xl border border-white/10 bg-slate-deep">
              <div className={`relative aspect-video bg-gradient-to-br ${f.gradient} transition duration-500 group-hover:scale-[1.02]`}>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-5 pt-16">
                  <span className="rounded-full border border-white/15 bg-black/40 px-2.5 py-0.5 text-[10px] uppercase tracking-wider text-stone-300">{f.tag}</span>
